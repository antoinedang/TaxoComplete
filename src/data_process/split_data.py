...
class Dataset:
    def __init__(self, graph_dataset, sampling_method, neg_number, seed, cosine_range):
        ...
        self.trainInputLevel, self.trainlevel2nodes = self._get_level(
            self.core_subgraph
        )
        ...
        self.train_node2pos, self.train_node2parents = self._find_insert_posistion(
            sampled_nodes, self.core_subgraph
        )
        self.trainInput = self._construct_training(
            sampled_nodes,
            self.train_node2pos,
            train_node_ids,
            sampling_method,
            neg_number,
            self.trainInputLevel,
        )
        # down_sample = random.sample(sampled_nodes, int(len(sampled_nodes) / 4))
        self.trainInputExampleLevel = self._construct_training_levels_v1(
            sampled_nodes,
            self.train_node2pos,
            train_node_ids,
            sampling_method,
        ...
        self.valid_node2pos, self.valid_node2parents = self._find_insert_posistion(
            graph_dataset.validation_node_ids, holdout_subgraph
        )
        self.val_examples = self._construct_validation(
            self.valid_node2pos, self.allLevel
        )
        # self.valInputExampleLevel = self._construct_training_levels(self.valid_node2pos, self.sample_nodes_level,
        # self.allLevel)
        self.dev_sentences1, self.dev_sentences2, self.dev_labels = (
            self._construct_validation_levels(self.valid_node2pos)
        )
        ...
        self.test_node2pos, self.test_node2parents = self._find_insert_posistion(
            graph_dataset.test_node_ids, holdout_subgraph
        )
        self.test_examples = self._construct_validation(
            self.test_node2pos, self.allLevel
        )
    ...
    def sample_matrix_level(self, trainlevel2nodes):
        ...
        for level in range(-1, max(trainlevel2nodes) + 1):
            ...
            nodes_level[level] = random.sample(
                nodes_in_level,
                (
                    int(len(nodes_in_level) / 4)
                    if len(nodes_in_level) > max(trainlevel2nodes)
                    else len(nodes_in_level)
                ),
            )
        return nodes_level
        ...
        for node in valid_node2pos:
            ...
            for pnode in s_parents:
                ...
                try:
                    val_examples.append(
                        InputExample(
                            guid=str(node) + "_" + str(pnode),
                            texts=[node_def, pnode_def],
                            label=[label_assign, valLevel[node], valLevel[pnode]],
                        )
                    )
                except:
                    pdb.set_trace()
    ...
    def _construct_training_levels_v1(
        self, sampled_nodes, train_node2pos, train_node_ids, sampling_method, neg_number
    ):
        pos_sample, neg_sample, pos_parent = self._construct_samples(
    ...
    def _construct_training_levels_v0(
        self, sampled_nodes, sample_nodes_level, trainInputLevel
    ):
        train_levels = []
        for node_s in sampled_nodes:
            ...
            pos_samples = random.sample(
                sample_nodes_level[level_node_s - 1],
                (
                    50
                    if len(sample_nodes_level[level_node_s - 1]) > 50
                    else len(sample_nodes_level[level_node_s - 1])
                ),
            )
            neg_samples_all = []
            ...
            neg_samples = random.sample(
                ...
                (
                    len(pos_samples) * 2
                    if len(neg_samples_all) > (len(pos_samples) * 2)
                    else len(neg_samples_all)
                ),
            )
            ...
            for node_l in pos_samples:
                ...
                train_levels.append(
                    InputExample(
                        guid=str(node_s) + "_" + str(node_l),
                        texts=[node_s_def, node_l_def],
                        label=[label],
                    )
                )
        return train_levels
    ...
    def _construct_samples(
        self, sampled_nodes, train_node2pos, train_node_ids, sampling_method, neg_number
    ):
        pos_sample, pos_parent = self._sample_positive(sampled_nodes, train_node2pos)
        if sampling_method.startswith("closest"):
            neg_sample = self._sample_negative_closest(
                sampled_nodes, pos_sample, train_node_ids, neg_number
            )
        else:
            neg_sample = self._sample_negative_random(
                sampled_nodes, pos_sample, train_node_ids, neg_number
            )
    ...
    def _sample_negative_random(
        self, sampled_nodes, pos_sample, train_node_ids, neg_number
    ):
        neg_rand_sample = {}
        for node in sampled_nodes:
            ...
            neg_rand_sample[node] = random.sample(
                sampling_set, min(len(sampling_set), len(s_pos) * neg_number)
            )
        return neg_rand_sample

    def _sample_negative_closest(
        self, sampled_nodes, pos_sample, train_node_ids, neg_number
    ):
        neg_rand_sample = {}
        sparse_nodes = 0
        for node in sampled_nodes:
            ...
            path_root_leaf = list(
                reversed(
                    nx.shortest_path(self.core_subgraph, source=self.root, target=node)
                )
            )
            ...
            if len(siblings) < len(s_pos):
                ...
                neg_rand_sample[node] = random.sample(
                    list_sample, min(len(s_pos) * neg_number, len(list_sample))
                )
                sparse_nodes += 1
            else:
                ...
                sampling_set_filtered = sampling_set.difference(set(s_pos)).difference(
                    set(siblings_sample)
                )
                random_sample_neg = random.sample(
                    sampling_set_filtered, min(len(sampling_set_filtered), close_len)
                )
    ...
    def _construct_corpus_nodes(self, queries_idx):
        return [
            self.definitions[elmt]["label"]
            + " [SEP] "
            + self.definitions[elmt]["summary"]
            for elmt in queries_idx
        ]
    ...
    def _construct_corpus(self, leaf_nodes_training):
        ...
        for node in leaf_nodes_training:
            ...
            words_root_leaf = " ".join(
                [self.definitions[elmt]["summary"] for elmt in path_root_leaf]
            )
